...
public class BaseRepositoryImpl<T, ID extends Serializable> implements
	 ...
	 */
	@Override
	public T getOneByHQL(String hqlString, Object... values) {
		Query query = this.getSession().createQuery(hqlString);
		if (values != null) {
			for (int i = 0; i < values.length; i++) {
				query.setParameter(String.valueOf(i), values[i]);
	 ...
	 */
	@Override
	public T getOneBySQL(String sqlString, Object... values) {
		Query query = this.getSession().createSQLQuery(sqlString);
		if (values != null) {
			for (int i = 0; i < values.length; i++) {
				query.setParameter(String.valueOf(i), values[i]);
	 ...
	 *      com.jipengblog.webadmin.repository.itv.launcher.util.RowMapper,
	 *      java.lang.Object[])
	 */
	@Override
